...
export async function generateStaticParams() {
    ...
    const static250Pokemons = data.results.map(pokemon => ({
        slug: pokemon.name,
    }))

    return static250Pokemons.map(({slug}) => (
        {
            slug: slug
        }
    ))
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    try {
        ...
        const { name } = await getPokemon(slug)
        ...
        return {
            ...
            description: `Lorem ipsum dolor, sit amet consectetur adipisicing elit. Vel, nam inventore autem non sunt quis, excepturi rerum, ipsam dolor provident earum ut cupiditate perferendis. Ipsa quibusdam dolorum laboriosam consectetur ipsam?`
        }
    }
}
...
const getPokemon = async (slug: string): Promise<Pokemon> => {
    ...
    try {
        ...
        const pokemon: Pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${slug}`, {
